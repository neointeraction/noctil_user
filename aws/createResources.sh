...
uploadS3Bucket() {
    ...
    npm run build $( if [ "$aws_cmd" == "awslocal" ]; then echo "--base-href /$BUCKET_NAME/"; fi )
    cd -
    echo "Syncing files to the S3 bucket from " $ROOT_DIR/build/
    $aws_cmd s3 sync $ROOT_DIR/build/ s3://$BUCKET_NAME/  --region $REGION
}
